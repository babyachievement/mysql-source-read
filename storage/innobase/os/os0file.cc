...
/** 表示某一类（ibuf,log,read,write）io handler所管理的innodb aio对象 */
struct os_aio_array_t{
	/* 对于read,write类型的os_aio_array_t可能会有多个线程并发访问
	*（innodb_read_io_threads、innodb_write_io_threads），
	对于ibuf,log类型因为它们都只有一个线程，所以不存在并发访问问题 */
	os_ib_mutex_t	mutex;	/*!< the mutex protecting the aio array */


	/** 
	* 一个条件变量event，当这个os_aio_array_t的slot
	* 从full变成not full时它释放该条件变量，
	* 即there is space in the aioarray；
	* 显然该变量会在提交io的时候关心 
	*/
	os_event_t	not_full;
	...
	/**
	* 另一个条件变量event，当这个os_aio_array_t的slot从not empty变成empty的时候，
	* 会释放该条件变量，
	* 也即no pending aio in the aioarray；
	*/
	os_event_t	is_empty;
				/*!< The event which is set to the
	...
	/* 该aio对象可容纳的pending aio event个数 也称为aio请求槽数*/
	/* 它等于该aio对象包括的所有线程（对于read,write类型则可能存在多个
	   线程共处于该对象的管理内）
	   可容纳的pending aio event个数 */
